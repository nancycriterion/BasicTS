...
class MyModelConfig(BasicTSModelConfig):
    ...
    cnn_out_channels: int = field(default=8, metadata={"help": "Number of output channels in CNN layers."})
    hidden_layers: int=field(default=3,metadata='')
    n_heads: int=field(default=2,metadata='')
    dropout: float=field(default=0.3,metadata='')
    nums_DMS: int=field(default=3,metadata='')
    hidden_sizes: list=field(default_factory=lambda: [4], metadata={"help": "GTUhidden feature sizes."})
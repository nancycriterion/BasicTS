...
from src.basicts.models.iTransformer import (iTransformerConfig,
                                         iTransformerForForecasting)

import torch

# 方法1.1：设置最大显存限制（单位：字节）
torch.cuda.set_per_process_memory_fraction(0.8)  # 最多使用50%显存
torch.cuda.empty_cache()  # 清空缓存



def test_smoke_test(dataset="ETTm1",num_features=7,output_len=336,num_epochs=5,input_len=96):
